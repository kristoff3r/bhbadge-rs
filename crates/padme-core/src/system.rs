...
use crate::{AudioSpeaker, Button, Error, Rom, Screen, SerialOutput};
...
impl<T: Deref<Target = [u8]>, SO: SerialOutput, AS: AudioSpeaker> System<T, SO, AS> {
    ...
    pub fn step<S: Screen>(&mut self, screen: &mut S) -> u8 {
        let ticks = self.cpu.step(&mut self.bus);

        for _ in 0..ticks {
            // self.bus.apu.step(&mut self.speaker);
            self.bus.ppu.step(screen, &mut self.bus.it);
            self.bus.timer.step(&mut self.bus.it);
        }

        // self.bus
        //     .serial
        //     .step(&mut self.serial_output, &mut self.bus.it);
    ...
    pub fn set_button(&mut self, button: Button, is_pressed: bool) {
        self.bus
            .joypad
            .set_button(button, is_pressed, &mut self.bus.it);
    }
    ...
    /// // }
    /// ```
    pub fn update_frame<S: Screen>(&mut self, screen: &mut S) -> u32 {
        let mut cycles = 0u32;
